...
vec4 performRaycasting(in vec3 entryPoint, in vec3 exitPoint, in vec2 texCoords) {
    ...
    while (t < tend) {
        ...
        vec3 gradient = computeGradient(_volume, _volumeTextureParams, samplePosition);//computeGradientCentralDifferencesLod(_volume, _volumeTextureParams, samplePosition, 1.0);

        // perform compositing
        vec4 color = lookupTF(_transferFunction, _transferFunctionParams, intensity);

        if (color.a > 0.0) {
            //vec3 gradient = computeGradientCentralDifferencesLod(_volume, _volumeTextureParams, samplePosition, 2.5);
            uint label = uint(texture(_labels, samplePosition).x * 255);
            float vesselness = texture(_vesselness, samplePosition).r;
            float snr = texture(_snr, samplePosition).r;
            float confidence = texture(_confidence, samplePosition).r;
            ...
            vec4 pr = performPredicateBasedShading(intensity, length(gradient) * 10.0, gradientAngle, viewAngle, snr, vesselness, confidence, label);
            float imp = pr.a;
